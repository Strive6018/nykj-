...
function ff_msubstr($str, $start=0, $length, $charset="utf-8", $suffix=true){
	if( function_exists('mb_strimwidth') ){
		if($suffix){
			return mb_strimwidth($str, 0, $length*2, '...', 'utf-8');
		}
		return mb_strimwidth($str, 0, $length*2, '', 'utf-8');
	}
...
function ff_tag_auto($title, $content){
	$data = ff_file_get_contents('/'.C('apikey_keyword').'&source='.rawurlencode(msubstr($content.$title,0,200)));
...
function ff_url_page($model, $params, $suffix=false, $pageid='pageid', $halfPer=5, $page = false){
	...
	$link='';
	if($page['currentpage'] > $halfPer){
		$link = '<li class="page-item"><a class="page-link" href="'.str_replace('FFLINK', 1, $jumpurl).'">1..</a></li>';
	}
	...
	for($i=$page['currentpage']-$halfPer,$i>1||$i=1,$j=$page['currentpage']+$halfPer,$j<$page['totalpages']||$j=$page['totalpages'];$i<$j+1;$i++){
		...
		if($i == 1){
			...
			if($page['currentpage']==1){
				$link .= '<li class="page-item disabled"><a class="page-link" href="'.$firrt_page.'">1</a></li>';
			}else{
				$link .= '<li class="page-item"><a class="page-link" href="'.$firrt_page.'">1</a></li>';
			}
		}else{
...
//导航自动处理函数
function ff_url_nav($nav_link){
	if( C('url_rewrite') && (C('url_html')==0) ){
		if(strpos($nav_link,'forum-index')){
			return ff_url('forum/index');
		}else if(strpos($nav_link,'special-index')){
			return ff_url('special/show');
		}else if(strpos($nav_link,'scenario-index')){
			return ff_url('scenario/index');
		}else{
			preg_match('/(news|vod)-show-id-([0-9]+)/i', $nav_link, $data);
			if($data){
				return ff_url_show($data[1].'/show', array('id'=>$data[2]) );
			}
		}
	}
	return $nav_link;
