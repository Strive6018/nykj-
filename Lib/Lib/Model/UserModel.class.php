 ...
 * Class UserModel
 * todo:: 数据库增加字段
 * todo:: user_phone(用户手机号码)
 * todo:: user_device_id(用户上次登陆设备id)
 * todo:: user_api_token(令牌验证)
 * todo:: user_api_token_time(上次生成令牌时间)
 * todo:: 新增检查api_token方法
 */
class UserModel extends RelationModel {
	...
	public function inspection_apitoken($api_token){
		...
		if(!S($api_token)){
            ...
            $info = $this->field('user_api_token,user_api_token_time,user_id')->where($where)->find();
            S($api_token,$info,ApiTokenCacheTime);
		}
        if(empty(S($api_token)['user_api_token']) || empty(S($api_token)['user_api_token_time'])){
            return ApiCodeApiTokenError;
        }
        $overtime = (int)S($api_token)['user_api_token_time']+VerificationCodeCacheTime*(3600*24);
        if($overtime <= time()){
            return ApiCodeOutOfDate;
        }
		return true;
	}
