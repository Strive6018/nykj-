<?php
//用于视图查询
class TagspecialViewModel extends ViewModel {
	//视图定义
	protected $viewFields = array (
		 'Tag'=>array('*'),
		 'Special'=>array('*', '_on'=>'Tag.tag_id = Special.special_id'),
	);
	// 查询多个数据
	public function ff_select_page($params, $where){
		...
		$infos = $this->field($params['field'])->where($where)->limit($params['limit'])->page($page['currentpage'])->order(trim($params['order'].' '.$params['sort']))->group('special_id')->select();
		// 是否写入数据缓存
		if($params['cache_name'] && $params['cache_time']){
	...
	public function ff_select_count($where,$params){
		if($params['group']){
			return $this->where($where)->count('distinct('.$params['group'].')');
		}else{
