...
class WeixinAction extends HomeAction{
	...
	private function handletext($object){
		...
		//字符串长度
		if(ff_mb_strlen($content) < 2 || ff_mb_strlen($content) > 8){
		...
		if($data = $this->select_vod($content)){
			...
			foreach($data as $key=>$value){
				if(C('wx_check') == 2){//返回播放页
					$play_max = ff_play_one(ff_play_list($value['vod_server'], $value['vod_play'], $value['vod_url']), 'max');
					$url_jump = C('wx_domain').'/index.php?s=/vod-play-id-'.$value['vod_id'].'-sid-'.$play_max['player_sid'].'-pid-'.$play_max['player_count'].'.html';
				}else{//详情页
					$url_jump = C('wx_domain').'/index.php?s=/vod-read-id-'.$value['vod_id'].'.html';
				}
				$xml .= "<item><Title><![CDATA[[".$value['list_name']."] ".$value['vod_name']."]]></Title><Description><![CDATA[".msubstr($value['vod_content'],0,200,true)."]]></Description><PicUrl><![CDATA[".ff_url_img($value['vod_pic'])."]]></PicUrl><Url><![CDATA[".$url_jump."]]></Url></item>";
			}
			return $this->response_item($object, $xml, count($data));
		}
		//无搜索结果
		if( C('wx_none_txt') ){
			if(C('wx_none_url')){
	...
	private function select_vod($keyword){
		...
		$params['limit'] = intval(C('wx_limit')); 
		if($params['limit'] > 8 || $params['limit'] < 1){ 
			$params['limit'] = 8; 
		}
