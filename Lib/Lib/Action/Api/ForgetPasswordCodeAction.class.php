...
class ForgetPasswordCodeAction extends ApiAction
{
    private $Sms;
    ...
    private function validate(){
        ...
        //检查手机号码是否已经注册
        if(empty($this->model['User']->field('user_phone')->where(['user_phone'=>$this->request['phone']])->find())){
            $this->api_error('该手机未注册');
        }
        //检查验证码是否已发送
