...
<div class="media">
  ...
  <div class="media-body">
    <h5 class="media-heading user-name">
      <a href="{:ff_url('user/index',array('id'=>$feifei['user_id']),true)}" target="_blank">{$feifei.user_name|htmlspecialchars|nb}</a>
			<gt name="forum_sid" value="0">
				...
				<small>《<a class="ff-text" href="{:ff_url('forum/vod',array('cid'=>$feifei['vod_id']),true)}">{$feifei.vod_name}</a>》</small>
				</gt>
			</gt>
    ...
    <p class="forum-btn">
      ...
      <a class="btn btn-default btn-xs forum-reply-get forum-reply-get-{$feifei.forum_reply}" data-id="{$feifei.forum_id}" href="{:ff_url('forum/detail', array('id'=>$feifei['forum_id']), true)}" title="评论详情"><span class="glyphicon glyphicon-align-right"></span> 详情</a>
    </p>
