<php>if($forum_cid){
	$news = D('news')->ff_find('*', array('news_id'=>array('eq',$forum_cid)), 'cache_page_news_'.$forum_cid, true);
}</php><!DOCTYPE html>
<html lang="zh-cn">
<head>
<include file="BlockBase:header_meta" />
<include file="BlockSeo:forum_detail_news" />
</head>
...
<ul class="breadcrumb mt-3 mb-0">
  ...
  <li class="breadcrumb-item active">{$news.news_name|msubstr=0,36,true}</li>
</ul>
<div class="card mt-3">
  <div class="card-body">
		<div class="card-text forum-content">
			{$forum_content|htmlspecialchars}
			@<a href="{:ff_url('forum/news',array('cid'=>$forum_cid),true)}">{$news.news_name}</a>
		</div>
