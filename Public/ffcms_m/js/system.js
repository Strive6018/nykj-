...
'mobile':{//移动端专用
	...
	'flickity':function(){//手机滑动
		if($(".ff-gallery1,ff-gallery2,ff-gallery3,ff-gallery4,ff-gallery5,ff-gallery6,ff-gallery7,ff-gallery8,ff-gallery9,ff-gallery10,ff-gallery11,ff-gallery12,ff-gallery13,ff-gallery14,ff-gallery15").length){
			$.ajaxSetup({ 
				cache: true 
			});
			$("<link>").attr({
			...
			$.getScript(cms.root+"Public/jquery.flickity/2.1.1/flickity.pkgd.min.js", function(){
				$(".ff-gallery1,ff-gallery2,ff-gallery3,ff-gallery4,ff-gallery5,ff-gallery6,ff-gallery7,ff-gallery8,ff-gallery9,ff-gallery10,ff-gallery11,ff-gallery12,ff-gallery13,ff-gallery14,ff-gallery15").each(function(i){
...
'language':{//简繁转换
	's2t':function(){
		if(feifei.browser.language=='zh-hk' || feifei.browser.language=='zh-tw'){
			$.getScript("//www.ffcms.cn/jquery/s2t/0.1.0/s2t.min.js", function(data, status, jqxhr) {
...
'alert':{//提示
	...
	'warning':function($id, $tips){
		$($id).html('<div class="aui-tips aui-margin-b-0 alert" id="tips-1"><div class="aui-tips-title">'+$tips+'</div><a href="#" class="aui-iconfont aui-icon-close" data-dismiss="alert"></a></div>');
	}
},
'image': {//图片
	...
	'qrcode': function(){//生成二维码
		...
		$(".glyphicon-phone").on('show.bs.popover', function () {
			$(".glyphicon-phone").attr('data-content','<img class="ff-qrcode" src="//www.ffcms.cn/qrcode/1.0/?w=150&h=150&url='+encodeURIComponent(feifei.browser.url)+'"/>');
		})
	},
	'vcode':function(){//安全码
...
'playurl': {//播放地址
	'download': function(){
		if($(".ff-playurl-down").length){
			$.getScript("//www.ffcms.cn/download/xunlei.js",function(){
				//3.4增加鼠标点击去掉文件名标题
	...
	'tongji': function(){
		if($("#cms_player").length){
			$.getScript("//www.ffcms.cn/tongji/3.8/");
		}
	},
	'active':function(){
