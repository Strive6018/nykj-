...
'cms':{//系统
	...
	'pre':function(){//代码高亮
		...
		if($("pre code").length){
			...
			$("<link>").attr({ rel: "stylesheet",type: "text/css",href: "/Public/css/min.css"}).appendTo("head");
...
'language':{//简繁转换
	's2t':function(){
		if(feifei.browser.language=='zh-hk' || feifei.browser.language=='zh-tw'){
			$.getScript("/Public/css/2t.min.js", function(data, status, jqxhr) {
...
'image': {//图片
	'lazyload': function(){//延迟加载
		...
		$.getScript(cms.root+"Public/ffcms/js/jquery.lazyload.min.js", function(response, status) {
			$("img.ff-img").lazyload({
	...
	'qrcode': function(){//生成二维码
		$(".glyphicon-phone").popover({html: true});
		$(".glyphicon-phone,.fn-download").on('show.bs.popover', function () {
			$(".glyphicon-phone").attr('data-content','<img class="ff-qrcode" src="//www.ffcms.cn/qrcode/1.0/?w=120&h=120&url='+encodeURIComponent(feifei.browser.url)+'"/>');
		})
	},
	'vcode':function(){//安全码
...
'search': {//搜索
	...
	'autocomplete': function(){//3.5增加搜索结果控制
		...
		if( $limit > 0){
			...
			$.getScript("/Public/ffcms/js/jquery.autocomplete.min.js", function(response, status) {
	...
	'get':function(){
		...
		if($(".ff-record-get").eq(0).length){
			$.get(cms.root+'index.php?g=home&m=record&a=vod&sid=1', function(data){
				if(data == ''){
					data = '<strong>您暂时还没有观看记录呦～</strong>';
				}
...
'score': {//评分
	'raty': function(){
		if( $('.ff-score').length ){
			...
			$("<link>").attr({ rel: "stylesheet",type: "text/css",href: "/Public/css/jquery.raty.min.css"}).appendTo("head");
			//
			$.getScript("/Public/css/jquery.raty.min.js", function(response, status) {
...
'share':{//分享
	'baidu': function(){
		if($(".ff-share").length ){
			...
			window._bd_share_config={common: {bdText: "",bdDesc: "",bdUrl: "",bdPic: ""},share: [{"bdSize": 24,bdCustomStyle: '/Public/ffcms/css/share.css'}]};with(document)0[(getElementsByTagName('head')[0]||body).appendChild(createElement('script')).src='http://bdimg.share.baidu.com/static/api/js/share.js?v=89860593.js?cdnversion='+~(-new Date()/36e5)];
		}
	}
},
'forum': {//讨论模块功能
...
'playurl': {//播放地址
	'download': function(){
		if($(".ff-playurl-down").length){
			$.getScript("/Public/ffcms/js/xunlei.js",function(){
				//3.4增加鼠标点击去掉文件名标题
